    ...
    <div className="min-h-screen bg-slate-950 text-white selection:bg-green-500 selection:text-slate-900">
      ...
      <div className="flex flex-col md:flex-row max-w-7xl mx-auto pt-24">
        ...
        <aside className="w-full md:w-64 shrink-0 md:border-r border-slate-800 md:min-h-[calc(100vh-6rem)] px-6 py-8 bg-slate-950 md:sticky md:top-24 self-start max-h-[calc(100vh-6rem)] overflow-y-auto">
          ...
          {/* 1. CLI-STUDIO (Green) */}
          <div className="mb-6">
            <Link href="/docs/cli-studio" className="flex items-center justify-between group mb-2">
                <span className={`font-bold text-sm uppercase tracking-wider ${isActive('/docs/cli-studio') ? 'text-green-400' : 'text-slate-500 group-hover:text-green-400 transition-colors'}`}>
                    CLI-Studio
                </span>
                {isActive('/docs/cli-studio') && <ChevronDown className="w-4 h-4 text-green-400" />}
            </Link>
            {isActive('/docs/cli-studio') && (
          ...
          {/* 3. SYNAPXIS (Amber/Orange) */}
          <div className="mb-6">
            <Link href="/docs/synapxis" className="flex items-center justify-between group mb-2">
                <span className={`font-bold text-sm uppercase tracking-wider ${isActive('/docs/synapxis') ? 'text-orange-400' : 'text-slate-500 group-hover:text-orange-400 transition-colors'}`}>
                    Synapxis
                </span>
                {isActive('/docs/synapxis') && <ChevronDown className="w-4 h-4 text-orange-400" />}
            </Link>
            {isActive('/docs/synapxis') && (
                <ul className="space-y-1 border-l border-slate-800 ml-2 animate-in slide-in-from-left-2 duration-200">
                    <li><Link href="/docs/synapxis#features" className="block px-4 py-1.5 text-sm text-slate-400 hover:text-white hover:border-l hover:border-white -ml-[1px]">Features</Link></li>
                    <li><Link href="/docs/synapxis#prerequisites" className="block px-4 py-1.5 text-sm text-slate-400 hover:text-white hover:border-l hover:border-white -ml-[1px]">Prerequisites</Link></li>
                    <li><Link href="/docs/synapxis#installation" className="block px-4 py-1.5 text-sm text-slate-400 hover:text-white hover:border-l hover:border-white -ml-[1px]">Installation</Link></li>
                    <li><Link href="/docs/synapxis#troubleshooting" className="block px-4 py-1.5 text-sm text-slate-400 hover:text-white hover:border-l hover:border-white -ml-[1px]">Troubleshooting</Link></li>
                </ul>
            )}
          </div>
          ...
          <div>
            ...
            <ul className="space-y-1 ml-2">
                <li><Link href="/docs/crolendar" className={`block px-4 py-1.5 text-sm ${isActive('/docs/crolendar') ? 'text-purple-400' : 'text-slate-400 hover:text-white'}`}>Crolendar</Link></li>
                ...
                <li><Link href="/docs/toxic" className={`block px-4 py-1.5 text-sm ${isActive('/docs/toxic') ? 'text-red-400' : 'text-slate-400 hover:text-white'}`}>Toxic</Link></li>
            </ul>
          </div>
